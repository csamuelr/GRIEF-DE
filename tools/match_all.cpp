...
int initializeDateset()
{
	...
	std::cout << "Dataset seems to be OK: " << seasons << " seasons and " <<  numLocations << " locations" << std::endl;
	

	printf("%p", offsetX);
	
	/*allocate variables*/
	offsetX = new int [numLocations*seasons];
	
	offsetY = new int [numLocations*seasons];
...
{
	Ptr<StarDetector> detector = StarDetector::create(45,0,10,8,5);
	Ptr<BriefDescriptorExtractor > descriptor = BriefDescriptorExtractor::create(32);
	srand (time(NULL));
	...
	for (int ims=0;ims<numLocations;ims++)
	{
		...
		for (int s = 0;s<seasons;s++)
		{
			...
			im[s] =  imread(filename, IMREAD_COLOR);
			...
			for (int a=0;a<seasons;a++){
				for (int b=a+1;b<seasons;b++){
					...
					if (drawAll || save || (draw&&(fabs(difference) > 35)))
					{
						...
						drawMatches(imA, kpA, imB, kpB, inliers_matches, img_matches, color, color, vector<char>(), cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
						...
						if (save){
							...
							putText(img_matches_transposed,description,cv::Point(10,28), FONT_HERSHEY_SIMPLEX, 0.75,color,2.5);

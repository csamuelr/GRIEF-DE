...
int main(int argc, char ** argv) 
{
	...
	do{
		...
		for (int s = 0;s<seasons;s++)
		{
			...
			im[s] =  imread(filename, cv::IMREAD_COLOR);
			img[s] = imread(filename, cv::IMREAD_GRAYSCALE);
			if (img[s].empty())
			{
		...
		for (int b=1;b<seasons;b++){
			...
			do{
				...
				if (kpA.size() >0 && kpB.size()>0 && inliers_matches.size() >0 && displayStyle == 2){
					...
					kpB.clear();
					matches.clear();
					drawMatches(imA, kpA, imB, kpB, matches, img_matches, Scalar(0, 0, 255), Scalar(0, 0, 255), vector<char>(), cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
				}
